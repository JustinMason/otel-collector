...
import (
	...
	"github.com/justinmason/otel-collector/receiver/prometheusremotewritereceiver/internal/errors"
	"github.com/justinmason/otel-collector/receiver/prometheusremotewritereceiver/internal/metrics"
)
...
func handleMetrics(resp http.ResponseWriter, req *http.Request, metricsReceiver *metrics.Receiver) {
	...
	encoder, ok := readContentType(resp, req)
	if !ok {
		return
	}
	...
	promReq, err := encoder.unmarshalWriteRequest(body)
	if err != nil {
		writeError(resp, encoder, err, http.StatusBadRequest)
		return
	}

	fmt.Printf("TimeSeries %v", promReq.Timeseries)

	//Ignore actual Response
	_, err = metricsReceiver.Write(req.Context(), &promReq)
	if err != nil {
		writeError(resp, encoder, err, http.StatusInternalServerError)
		return
	}

	writeResponse(resp, encoder.contentType(), http.StatusOK, nil)
}

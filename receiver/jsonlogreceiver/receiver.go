...
package jsonlogreceiver // import "github.com/justinmason/otel-collector/receiver/prometheusremotewritereceiver"
...
func newRemoteWriteReceiver(set *receiver.Settings, cfg *Config, next consumer.Logs) (*remoteWriteReceiver, error) {
...
func (r *remoteWriteReceiver) startHTTPServer(ctx context.Context, host component.Host) error {
	...
	if r.consumer != nil {
		httpMetricsReceiver := logs.New(r.consumer, r.obsrepHTTP, r.cfg.LogTypes)
		httpMux.HandleFunc(r.cfg.HTTP.LogsURLPath, func(resp http.ResponseWriter, req *http.Request) {
			handleLogs(resp, req, httpMetricsReceiver)
		})
	}
...
func (r *remoteWriteReceiver) registerLogsConsumer(mc consumer.Logs) {
	r.consumer = mc
}
